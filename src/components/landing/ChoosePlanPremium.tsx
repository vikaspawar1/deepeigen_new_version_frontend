...
function ChoosePlanPremium() {
    ...
    const planParam = searchParams.get("plan") || state?.planName?.toLowerCase() || "premium"
    ...
    const handlePayment = async () => {
        if (!user) {
            ...
            navigate("/login", { state: { from: `/choosepremium?plan=${planParam}&duration=${durationParam}` } });
            return;
        }
    ...
    const displayName = planData?.plan || state?.planName || "Premium"
            ...
            <div className=" w-full">
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 p-4 sm:p-6 md:p-8 lg:p-10">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8 border-b border-gray-400 pb-6">
                        <div>
                            ...
                            <p className="text-sm sm:text-base text-gray-500 mt-1">
                                {state?.subtitle || (planParam === "premium" ? "Complete curriculum & all premium courses" : "")}
                            </p>
                        </div>
                    ...
                    <button
                        ...
                        className="w-full mt-2 bg-blue-600 text-white py-3 sm:py-4 rounded-xl text-base sm:text-lg font-medium hover:bg-blue-700 active:scale-[0.98] transition disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                        {paymentLoading ? (
                            <>
...
export default ChoosePlanPremium