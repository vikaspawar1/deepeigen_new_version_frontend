...
function ChoosePlanCard() {
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        setLoading(true)
        const { data, status } = await api(`subscriptions/plan-details/${planParam}/${durationParam}/`)
      ...
      <div className=" w-full">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 p-4 sm:p-6 md:p-8 lg:p-10">
          ...
          <button
            ...
            disabled={loading}
            className="w-full mt-2 bg-blue-600 text-white py-3 sm:py-4 rounded-xl text-base sm:text-lg font-medium hover:bg-blue-700 active:scale-[0.98] transition disabled:opacity-50"
          >
