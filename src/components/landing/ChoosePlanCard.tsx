...
const durationOptions = [
  ...
  { value: "Quarterly", label: "4 Months" },
...
function ChoosePlanCard() {
  ...
  const planParam = searchParams.get("plan") || state?.planName?.toLowerCase() || "basic"
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        setPageLoading(true)
        const { data, status } = await api(`subscriptions/plan-details/${planParam}/${selectedDuration}/`)
  ...
  const handlePayment = async () => {
    if (!user) {
      ...
      navigate("/login", { state: { from: `/chooseplan?plan=${planParam}&duration=${durationParam}` } });
      return;
    }
  ...
  setPaymentLoading(false);
    ...
    try {
      ...
      // 1.5 Handle free subscription (full deduction removed, but keeping logic in case order total is 0)
      if (orderData.free) {
...
if (pageLoading) {
  ...
  const getFriendlyDuration = () => {
    ...
    if (d === "FOUR_MONTH" || d === "QUARTERLY" || d === "4 MONTHS") return "4 Months"
    if (d === "YEARLY" || d === "1 YEAR") return "1 Year"
    return rawDuration
  }

  const displayName = planData?.plan || state?.planName || "Basic"
    ...
    <div className=" font-bricolage  max-w-full sm:max-w-[84vw] md:max-w-[90vw] lg:max-w-[86vw] mx-auto min-h-screen bg-[#e9effb] px-3 sm:px-4 md:px-8 lg:px-12 py-4 sm:py-6 md:py-10">
      <div className=" w-full">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 p-4 sm:p-6 md:p-8 lg:p-10">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8 border-b border-gray-400 pb-6">
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-500 mt-1">
                {state?.subtitle || (planParam === "basic" ? "Essential & foundation courses" : "")}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="inline-flex bg-white border border-gray-200 rounded-full p-2">
                {durationOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold rounded-full transition-all duration-200 cursor-pointer ${
                        isActive
          ...
          {/* Lectures */}
          <div className="mb-6">
            <div className="max-h-96 sm:max-h-64 md:max-h-72 overflow-y-auto space-y-2 pr-1 sm:pr-2">
              {lectures.map(l => (
                <div
                  key={l.id}
  ...
  className="w-full mt-2 bg-blue-600 text-white py-3 rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
>
  {paymentLoading ? (
    <>
